...
public class TcpConnectionListener implements Runnable {
  ...
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(TcpConnectionListener.class);
    ...
    while (running) {
      try {
        ...
        TcpClientHandler clientHandler =
            new TcpClientHandler(clientSocket, this.subscriber, context, running);
        ...
        for (TcpClientHandler handler : handlers) {
          if (handler.isRunning()){
            logger.info(handler.getClientAddress() + " connected");
          } else {
  ...
  public TcpConnectionListener(
      EventStreamContext context,
      ServerSocket serverSocket,
      AtomicReference<EventStreamSource.Subscriber> subscriber,
      Boolean running) {
